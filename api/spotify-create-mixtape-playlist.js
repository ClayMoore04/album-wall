export default async function handler(req, res) {
  ...
  const { access_token, playlist_name, description, is_public, track_ids } =
    req.body;

  if (!access_token || !playlist_name || !track_ids?.length) {
    return res
      .status(400)
      .json({ error: "Missing access_token, playlist_name, or track_ids" });
  }
  ...
  try {
    ...
    const createRes = await spotifyFetch(
      `https://api.spotify.com/v1/users/${me.id}/playlists`,
      {
        ...
        body: JSON.stringify({
          name: playlist_name,
          description: description || "Created from Album Wall Mixtapes",
    ...
    console.error("Create mixtape playlist error:", e);
